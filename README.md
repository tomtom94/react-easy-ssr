...
- You won't ever be able to use `renderToNodeStream` with Next.js forget it. You will only be able to use the old one React function `renderToString`, [so bad just check this github issue it on the roadmap](https://github.com/vercel/next.js/issues/1209). To me you can't make a switch to use one function or the other just with a boolean parameter true or false. It'll need you to change a part of the architecture of you app, and also some module you are using may not be compatible (`redux-saga` and `react-jss` for example)
